...
export default function IncreasingLayout() {
  const {
  ...
  const LOADING_POPULATION =
    loadingPopulation('onGetPopulationIncreasing') || false

  const onChangePeriode = (periode: Moment) => {
    if (!periode) return null
    return actionPopulation<'onGetPopulationIncreasing'>(
    ...
    <Flex vertical gap={12} justify="space-between" className="w-full h-full">
      <Flex justify="space-between" align="center">
        <p className="underline text-[1.5em] font-semibold text-stone-600">
          TOP 5 Increasing Population
        </p>
        <LynxDatePicker.MonthPicker
          className="w-[200px]"
          size="large"
          disabled={LOADING_POPULATION}
          value={moment(statePopulation.increasingFilter.periode)}
          format="MMM YYYY"
      ...
      <List
        ...
        dataSource={statePopulation.listIncreasing}
        bordered
          ...
          >
            ...
            <Flex justify="space-between" align="center" className="w-full">
              ...
              <Flex gap={2} vertical align="end" className="leading-[1.25]">
                ...
                />
                <LynxCurrency
                  value={item.increasing as number}
                  prefix="+"
                  fraction={{ max: 2 }}
                  locale="en-US"
                  className="font-semibold text-green-600 text-[14px]"
                />
              </Flex>
            </Flex>
          </Flex>
